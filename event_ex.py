...
class eventCls: # class to handle event data and extension table
    def __init__(self, tuple): # tuple is the tuple returned from unpickle blob_data in event table
        ...
        self.date_str = tuple[3][4]
        self.description = tuple[2]
        self.event_type = event_types[tuple[2][0]]
        self.gramps_id = tuple[1]
        self.handle = tuple[0]
        self.place_handle = tuple[5]
        self.private = tuple[12]       
    def __str__(self):
        return f"{self.event_type} {self.description} {self.date_str} place:{self.place_handle}"
    sql_insert_txt = "INSERT INTO event_ex (handle, gramps_id, date_str, description, place_handle, change, private) values (?,?,?,?,?,?,?)"
...
for row in cur.execute("SELECT handle, blob_data, description FROM event LIMIT 10 "):
